...
def resp_handler(message, _):
    state = userstate.get_state(message.chat.id, "callback", False)
    if state: 
        ...
        userstate.update_state(message.chat.id, "callback", False)
        userstate.update_state(config["owner_id"], "need_res", True)
        return True
...
def owner_respons(message:Message, _):
    if message.reply_to_message:
        ...
        userstate.update_state(config["owner_id"], "need_res", False)
