...
def load_image(file_path):
    if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
        with open(file_path, 'rb') as f:
            return f.read()
    raise FileNotFoundError(f"Файл {file_path} не найден или пуст")
...
def types_send_nlc(call, query):
    markup = types.InlineKeyboardMarkup(
        [[types.InlineKeyboardButton("Выбрать другой тип", callback_data="type")],
...
def type_choise(call, query):
    edit(call.message)
    bot.send_message(
        call.message.chat.id, "Выберите тип, о котором хотите узнать", reply_markup=types.InlineKeyboardMarkup(
