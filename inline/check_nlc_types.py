    ...
    if query[0] == "check":
        try:
            if query[1] == "return":
                ...
                userstate.update_state(call.message.chat.id, "types nlc", True)
                userstate.update_state(call.message.chat.id, "id", mes.id)
                return          
        except:
        ...
        state = userstate.update_state(call.message.chat.id, "types nlc", False)
        userstate.update_state(call.message.chat.id, "id", None)
        bot.delete_message(call.message.chat.id, call.message.id)
